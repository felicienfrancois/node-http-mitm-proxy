...
Proxy.prototype.listen = function(options) {
  ...
  this.http2Enabled = !!options.http2Enabled;
  if (this.http2Enabled && !this.silent) {
    console.log('Experimental HTTP/2 support enabled');
  }
  this.httpAgent = options.httpAgent || new http.Agent();
  this.httpsAgent = options.httpsAgent || new (this.http2Enabled ? http2 : https).Agent();
  ...
  function openHttpsServer(hostname, callback) {
    self.onCertificateRequired(hostname, function (err, files) {
      async.auto({
        ...
        var httpsServer = (self.http2Enabled ? http2 : https).createServer(results.httpsOptions);
...
Proxy.prototype._onWebSocketServerConnect = function(isSSL, ws) {
  ...
  ctx.proxyToServerWebSocketOptions = {
    ...
    headers: self._filterHeaders(ctx.clientToProxyWebSocket.upgradeReq.headers)
  };
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  ctx.proxyToServerRequestOptions = {
    ...
    host: hostPort.host,
    port: hostPort.port,
    headers: self._filterHeaders(ctx.clientToProxyRequest.headers),
    agent: ctx.isSSL ? self.httpsAgent : self.httpAgent
  };
  ...
  function makeProxyToServerRequest() {
    var proto = ctx.isSSL ? (self.http2Enabled ? http2 : https) : http;
  ...
  function proxyToServerRequestComplete(serverToProxyResponse) {
    ...
    return self._onResponse(ctx, function(err) {
      ...
      return self._onResponseHeaders(ctx, function (err) {
        ...
        ctx.proxyToClientResponse.writeHead(ctx.serverToProxyResponse.statusCode, self._filterHeaders(ctx.serverToProxyResponse.headers));
...
Proxy.prototype._filterHeaders = function(headers) {
  if (!headers) return headers;
  var r = {};
  for (var key in headers) {
    var canonizedKey = key.trim().toLowerCase();
    // filter websocket headers
    if (canonizedKey.indexOf('sec-websocket') === 0) {
      continue;
    }
    // filter http2 deprecated headers
    if (this.http2Enabled && ['connection','host','keep-alive','proxy-connection','transfer-encoding','upgrade'].indexOf(canonizedKey) !== -1) {
      continue;
    }
    r[canonizedKey] = headers[key];
  }
  return r;
};
